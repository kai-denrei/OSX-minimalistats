...
| 2026-06-29 | Safe slim pass: drop 1024px + 2 dup app icons; strip all non-en localizations (en-only) | Repo 9.7→7.4 MB. No storyboards/Base, so en-only is safe; build green. | [[ux]] |
| 2026-06-29 | Distribute via git-clone + local build, NOT prebuilt release builds | A prebuilt ad-hoc app trips Gatekeeper on another Mac (notarization = $99/yr Apple acct); clone + local build ad-hoc-signs per machine, no friction. Both Macs have Xcode. | [[pm]] |
...
| 2026-06-29 | Listed "thin to arm64-only" as an app-size win | WRONG — Debug builds already default to `ONLY_ACTIVE_ARCH=YES`; every binary verified arm64 (no x86_64 slice). Universal only applies to Release/archive builds, which we don't make. Zero gain. |
...
- Debug `xcodebuild` is already arm64-only (`ONLY_ACTIVE_ARCH=YES`) — "thin to arm64" is a no-op locally. The installed app's (~19 MB) real fat is Debug symbols (a Release build strips them, ~→11 MB) + the 4 functionally-removed module frameworks (Net/Remote/Clock/Bluetooth ~3.7 MB) STILL embedded, because only code-level removal was done, not target removal. — from 2026-06-29
...
## Session Log
- 2026-06-29 (sync) — slim pass done (icons + en-only localizations → 7.4 MB); arm64 already done (no gain); decided clone-over-release distribution
